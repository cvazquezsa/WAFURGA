...
--SELECT * FROM #StockMinimo WHERE ARTICULO='FL900'
  ...
  -- SELECT * FROM ResumenCantTraspaso WHERE ARTICULO='FL900' order by Sucursal
   ...
   --select * from #WFGCantidadTraspasar where articulo='fl900'
...
@SucursalAnt	int
   ...
   BEGIN        
	 ...
	 WHILE EXISTS (SELECT * FROM #WFGCantidadTraspasar WHERE (Existencia < Stock OR Existencia < InvXDia) AND InvXDia >0 AND Ciclo=@CicloMax AND Articulo=@Articulo AND ISNULL(Subcuenta,'')=ISNULL(@Opcion,'') AND ISNULL(Excluido,0)=0) 
	 BEGIN
	   SELECT TOP 1 @Sucursal=Sucursal, @InvDia=Existencia, @InvXDiaR=InvXDia FROM #WFGCantidadTraspasar WHERE (Existencia < Stock OR Existencia < InvXDia) AND InvXDia >0 AND Ciclo=@CicloMax AND Articulo=@Articulo AND ISNULL(Subcuenta,'')=ISNULL(@Opcion,'') 
       ORDER BY CalifFinal
	    IF @CicloMax >=2	   
	    SELECT  @InvXDiaAnt=w1.InvXDia-w.InvXDia /*w.ciclo,w1.ciclo, w1.invxdia, w1.sucursal*/
	    ...
	    WHERE w.Ciclo=1 AND w1.Ciclo=@CicloMax AND w1.Sucursal=@Sucursal
	   SELECT @InvDia=@InvDia+isnull(@InvXDiaAnt,0)
	   
	   SELECT @TotalInvXDia=SUM(ISNULL(InvXDia,0)) FROM #WFGCantidadTraspasar WHERE Sucursal <> @Sucursal AND Ciclo=@CicloMax AND Articulo=@Articulo AND ISNULL(Subcuenta,'')=ISNULL(@Opcion,'')
	   --SELECT @TotalInvXDia=@TotalInvXDia
	   ...
	   UPDATE #WFGCantidadTraspasar SET Proporcion=InvXDia/ISNULL(@TotalInvXDia,0) WHERE Sucursal <> @Sucursal AND Ciclo=@CicloMax and Articulo=@Articulo and isnull(Subcuenta,'')=isnull(@Opcion,'')

	   UPDATE #WFGCantidadTraspasar set excluido=1 where Sucursal=@Sucursal and Ciclo=@CicloMax and Articulo=@Articulo and isnull(Subcuenta,'')=isnull(@Opcion,'')
	   UPDATE #WFGCantidadTraspasar set excluido=0 where NOT(Sucursal=@Sucursal) and Ciclo=@CicloMax and Articulo=@Articulo and isnull(Subcuenta,'')=isnull(@Opcion,'')
	   SELECT @CicloMax=@CicloMax+1

	  
		--select @InvDia
	   INSERT INTO #WFGCantidadTraspasar (Sucursal, Articulo, Subcuenta, Existencia, CalifFinal, InvXDia,                                                                               Ciclo,     Stock)
	   	                           SELECT sucursal, articulo, Subcuenta, Existencia, CalifFinal, CASE WHEN ISNULL(excluido,0)=1 THEN 0 ELSE ((@invdia*ISNULL(Proporcion,0))+InvXDia) END, @CicloMax, Stock
								   FROM #WFGCantidadTraspasar WHERE Ciclo=@CicloMax-1 AND Articulo=@Articulo AND ISNULL(Subcuenta,'')=ISNULL(@Opcion,'')
       
	   SELECT @SucursalAnt=@Sucursal
	   --SELECT * FROM #WFGCantidadTraspasar WHERE (existencia < stock OR existencia < InvXDia) AND InvXDia >0 AND Ciclo=@CicloMax AND Articulo=@Articulo AND  ISNULL(Subcuenta,'')=ISNULL(@Opcion,'') AND ISNULL(Excluido,0)=0
     END  
   FETCH NEXT FROM crCantPos1 INTO  @Articulo, @Opcion
   END    
   CLOSE crCantPos1
   ...
   --select * from #CicloMax WHERE ARTICULO='FL900'
   ...
   SELECT w.Articulo, w.Subcuenta, w.Sucursal, w.Existencia, w.califFinal, w.InvXDia, w.Ciclo, w.Stock, null
   FROM #WFGCantidadTraspasar w 
   ...
   --SELECT * FROM #Recibir1 WHERE ARTICULO='FL900'
   ...
   --SELECT * FROM #Recibir3 WHERE ARTICULO='FL900'

   --select * from #WFGCantTraspasaCicloMax
   declare @TotalExistInvXDia0 float,
		   ...
		   --select * from #WFGCantTraspasaCicloMax WHERE Articulo='FL900'
   ...
   BEGIN
	 ...
	 --IF @Articulo='0606' AND @SubCuenta='c4T1'
		--select 1,* from #WFGCantTraspasaCicloMax WHERE Articulo=@Articulo AND Subcuenta=@Subcuenta
	 
	 FETCH NEXT FROM crPlaneador INTO @Articulo, @Subcuenta
     END
	 CLOSE crPlaneador
     DEALLOCATE crPlaneador	 
	 --select * from #WFGCantidadTraspasar
 ...
 DECLARE @CalifFinalMax float,
	 ...
	 --select * from #SumaCantDec where articulo='fl900'

	 --select sucursal, TraspasaASuc, RecibeTraspaso, CantidadATraspasar, * from WFGPlaneadorTraspaso where articulo='0606' and subcuenta='c4t1' order by 1 
	 ...
	 --select sucursal, TraspasaASuc, RecibeTraspaso, CantidadATraspasar, * from WFGPlaneadorTraspaso where articulo='fl900' --and subcuenta='c4t1' order by 1 
	 /* R E P O R T E*/
	 ...
	 INSERT INTO #Disponible
		 SELECT  Sucursal, Articulo, Subcuenta, Existencia-InvXDia
		 FROM #WFGCantTraspasaCicloMax
		 WHERE CantidadTraspasar=0

		--SELECT * FROM #Disponible where articulo='fl900'
