...
Visible=S
Expresion=Si(MovTipo('COMS',Compra:Compra.Mov)=COMS.O, ReporteImpresora('OrdenCompra'), ReporteImpresora(ReporteMovPantalla('COMS', Compra:Compra.Mov, Compra:Compra.Estatus), Compra:Compra.ID))
...
UsaTeclaRapida=S
TeclaRapida=Alt+F11
Expresion=Si(MovTipo(<T>COMS<T>,Compra:Compra.Mov)=COMS.O, ReportePantalla(<T>OrdenCompra<T>), ReportePantalla(ReporteMovPantalla(<T>COMS<T>, Compra:Compra.Mov, Compra:Compra.Estatus), Compra:Compra.ID))
...
CompraD.ArticuloMaquila=PrecioListaArt
CostoMXN=CompraD.ValorAduana
PrecioListaArt=(Fin)
...
Antes=S
NombreEnBoton=S
Expresion=Asigna(Info.Articulo, Nulo)<BR>Forma(<T>WFGArtExpress<T>)<BR>Si <BR>  ConDatos(Info.Articulo) y (Compra:Compra.Estatus en (EstatusSinAfectar, EstatusPorConfirmar))<BR>Entonces<BR>  Asigna(CompraD:CompraD.Articulo, Info.Articulo)<BR>Fin
AntesExpresiones=Asigna(Info.Id,Compra:Compra.Id)
VisibleCondicion=MovTipoEn(<T>COMS<T>, Compra:Compra.Mov,(COMS.O, COMS.R))

[Acciones.WFGEtiquetas]
Nombre=WFGEtiquetas
Boton=6
NombreEnBoton=S
...
AntesExpresiones=Asigna(Info.ID, Compra:Compra.ID)<BR>Asigna(Info.Articulo, CompraD:CompraD.Articulo)<BR>Asigna(Info.RenglonID, CompraD:CompraD.RenglonID)<BR>Asigna(Info.Opcion, CompraD:CompraD.SubCuenta)
VisibleCondicion=MovTipoEn(<T>COMS<T>, Compra:Compra.Mov,(COMS.F, COMS.EG, COMS.EI))

[Acciones.WFGEtiquetas.Variables]
Nombre=Variables
Boton=0
TipoAccion=Controles Captura
...
[Acciones.WFGEtiquetas.Preliiminar]
Nombre=Preliiminar
Boton=0
TipoAccion=Reportes Pantalla
ClaveAccion=WFGEtiquetas
...
[Acciones.WFGEtiquetas.ListaAccionesMultiples]
(Inicio)=Variables
Variables=Preliiminar
Preliiminar=(Fin)
...
Multiple=S
ListaAccionesMultiples=(Lista)
AntesExpresiones=Asigna(Info.ID, Compra:Compra.ID)<BR>Asigna(Info.Articulo, CompraD:CompraD.Articulo)<BR>Asigna(Info.RenglonID, CompraD:CompraD.RenglonID)<BR>Asigna(Info.Opcion, CompraD:CompraD.SubCuenta)<BR>Asigna(Info.Empresa, Compra:Compra.Empresa)<BR>Forma(<T>WfgActNP<T>)
VisibleCondicion=MovTipoEn(<T>COMS<T>, Compra:Compra.Mov,(COMS.F, COMS.EG, COMS.EI))

[Acciones.WFGPrecio.Variables]
Nombre=Variables
Boton=0
TipoAccion=Controles Captura
ClaveAccion=Variables Asignar
...
[Acciones.WFGPrecio.Preliiminar]
Nombre=Preliiminar
Boton=0
TipoAccion=Expresion
ClaveAccion=Forma(<T>WfgActNP<T>)
Activo=S
Visible=S

[Acciones.WFGPrecio.ListaAccionesMultiples]
(Inicio)=Variables
Variables=Preliiminar
Preliiminar=(Fin)

[Detalle.PrecioListaArt]
Carpeta=Detalle
Clave=PrecioListaArt
Editar=N
LineaNueva=S
ValidaNombre=S
3D=S
